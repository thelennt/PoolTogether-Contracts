...
abstract contract PrizePool is PrizePoolInterface, OwnableUpgradeable, ReentrancyGuardUpgradeable, TokenControllerInterface, IERC721ReceiverUpgradeable {
    ...
    canAddLiquidity(amount)
  {
    ...
    uint256 modifiedAmount = amount.sub(_getEntryFee(amount));
    _mint(to, modifiedAmount, controlledToken, referrer);

    _token().safeTransferFrom(operator, address(this), amount);
    _supply(modifiedAmount);

    emit Deposited(operator, to, controlledToken, amount, modifiedAmount, referrer);
  }
  ...
  /// @notice Limits the exit fee to the maximum as hard-coded into the contract
  /// @param amount The amount that is attempting to be withdrawn
  /// @return the entrance fee
  function _getEntryFee(uint256 amount) internal view returns (uint256) {
    return amount.div(entryFee);
  }
